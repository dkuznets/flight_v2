...
/* Snandard & Extended ID counters currently stored in acceptance filter array */
...
// <o0> CPU Type <0=> LPC2294 or LPC2294/00 <1=> LPC2294/01
//      <i> This feature affects the way the interface number is presented in the Global Acceptance Filter.
//      <i> In the old CAN block (LPC2294 and LPC2294/00), the numbering of the CAN interfaces in the LUT is 1 to N.
//      <i> In the new CAN block (LPC2294/01), the numbering of the CAN interfaces in the LUT is 0 to N-1.
//      <i> (where N is a number of CAN interfaces available in the CPU)
#define	ENHANCED_CAN_CONTROLLER	1
...
CAN_ERROR CAN_hw_init (U32 ctrl, U32 baudrate)  {
  ...
  CAN_std_cnt = 0;                    /* Reset standard & extended ID counters*/
  CAN_ext_cnt = 0;                    /* to store array with new IDs         */
...
/*----------------------- CAN_hw_tx_empty_keep_state -------------------------
 *
 *  Check if controller is available for transmission, but not decrementing
 *  the write semaphore token (by A.Kozyrev)
 *
 *  Parameter:  ctrl:       Index of the hardware CAN controller (1 .. x)
 *
 *  Return:     CAN_ERROR:  Error code
...
CAN_ERROR CAN_hw_tx_empty_keep_state (U32 ctrl)  {
  U32 ctrl0 = ctrl-1;                 /* Controller index 0 .. x-1           */
  regCAN *ptrcan = (regCAN *) CAN_BASE[ctrl0];

  if ((os_sem_wait (wr_sem[ctrl0], 0) != OS_R_TMO)){ /* If semaphore is free*/

    os_sem_send(wr_sem[ctrl0]);                   /* Return a token back to semaphore    */

    if (ptrcan->CANSR & 0x00000004)               /* Transmitter ready for   */
      return CAN_OK;                              /* transmission            */
  }
...
/*--------------------------- CAN_hw_tx_abort -------------------------------
 *
 *  Abort transmittion (by A.Kozyrev)
 *
 *  Parameter:  ctrl:       Index of the hardware CAN controller (1 .. x)
 *
 *  Return:     CAN_ERROR:  Error code
...
CAN_ERROR CAN_hw_tx_abort(U32 ctrl)
{
	U32 ctrl0 = ctrl-1;                 /* Controller index 0 .. x-1           */
 	regCAN *ptrcan = (regCAN *) CAN_BASE[ctrl0];
	...
	if((os_sem_wait(wr_sem[ctrl0], 0) != OS_R_TMO)) /* If semaphore is free */
	{
		// nothing to abort, just exit
	    os_sem_send(wr_sem[ctrl0]);                 /* Return a token back to semaphore    */
		return CAN_OK;
	}

    if(!(ptrcan->CANSR & 0x00000004))               /* If transmitter is busy       */
	{
		ptrcan->CANCMR |= 0x00000002;               /* Abort Transmittion        */

		while(!(ptrcan->CANSR & 0x00000004));	    /* Wait untill transmitter becomes free */

		if(os_sem_wait(wr_sem[ctrl0], 0xffff) != OS_R_TMO) /* Wait for the token to be returned (by CAN_TXn_ISR) */
		    os_sem_send(wr_sem[ctrl0]);             /* Return a token back to semaphore    */
	}
...
/*--------------------------- CAN_hw_rx_get_err_counter ----------------------
 *
 *  Read current RXERR counter register (by A.Kozyrev)
 *
 *  Parameter:  ctrl:       Index of the hardware CAN controller (1 .. x)
 *
 *  Return:     U8:         Number of RX errors
 *---------------------------------------------------------------------------*/

U8 CAN_hw_rx_get_err_counter(U32 ctrl)
{
  U32 ctrl0 = ctrl-1;                 /* Controller index 0 .. x-1           */
  regCAN *ptrcan = (regCAN *)CAN_BASE[ctrl0];

  return ((ptrcan->CANGSR & 0x00ff0000) >> 16);
}

/*--------------------------- CAN_hw_tx_get_err_counter ----------------------
 *
 *  Read current TXERR counter register (by A.Kozyrev)
 *
 *  Parameter:  ctrl:       Index of the hardware CAN controller (1 .. x)
 *
 *  Return:     U8:         Number of TX errors
 *---------------------------------------------------------------------------*/

U8 CAN_hw_tx_get_err_counter(U32 ctrl)
{
  U32 ctrl0 = ctrl-1;                 /* Controller index 0 .. x-1           */
  regCAN *ptrcan = (regCAN *)CAN_BASE[ctrl0];

  return ((ptrcan->CANGSR & 0xff000000) >> 24);
}

/*------------------------ CAN_hw_tx_reset_err_counter ------------------------
 *
 *  Reset TX error counter register (by A.Kozyrev)
 *
 *  Parameter:  ctrl:       Index of the hardware CAN controller (1 .. x)
 *
 *  Return:     -
...
void CAN_hw_tx_reset_err_counter(U32 ctrl)
{
  U32 ctrl0 = ctrl-1;                 /* Controller index 0 .. x-1           */
  regCAN *ptrcan = (regCAN *)CAN_BASE[ctrl0];
  U32 interrupts;
  ...
  ptrcan->CANMOD |=  (1<<0);          /* enter Reset mode                    */
  ptrcan->CANGSR &= ~(0xff000000);    /* reset TX error counter bits		 */
  ptrcan->CANMOD &= ~(1<<0);          /* enter Normal operating mode         */

  VICIntEnable = interrupts;
}

/*------------------------ CAN_hw_rx_reset_err_counter------------------------
 *
 *  Reset RX error counter register (by A.Kozyrev)
 *
 *  Parameter:  ctrl:       Index of the hardware CAN controller (1 .. x)
 *
 *  Return:     -
...
void CAN_hw_rx_reset_err_counter(U32 ctrl)
{
  U32 ctrl0 = ctrl-1;                 /* Controller index 0 .. x-1           */
  regCAN *ptrcan = (regCAN *)CAN_BASE[ctrl0];
  U32 interrupts;
  ...
  ptrcan->CANMOD |=  (1<<0);          /* enter Reset mode                    */
  ptrcan->CANGSR &= ~(0x00ff0000);    /* reset RX error counter bits         */
  ptrcan->CANMOD &= ~(1<<0);          /* enter Normal operating mode         */

  VICIntEnable = interrupts;
}

/*------------------------ CAN_hw_clear_reset_mode ---------------------------
 *
 *  Clears RM (reset mode) bit in CANMOD register (by A.Kozyrev)
 *
 *  Parameter:  ctrl:       Index of the hardware CAN controller (1 .. x)
 *
 *  Return:     -
...
void CAN_hw_clear_reset_mode(U32 ctrl)
{
  U32 ctrl0 = ctrl-1;                 /* Controller index 0 .. x-1           */
  regCAN *ptrcan = (regCAN *)CAN_BASE[ctrl0];
...
#if	ENHANCED_CAN_CONTROLLER == 1
	ctrl = ctrl - 1;	// modify for LPC2294/01
#endif
